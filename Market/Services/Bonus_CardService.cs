...
namespace Market.Services
{
    public class Bonus_CardService:IBonus_Card
    {
        ...
        public async Task<ICollection<Bonus_CardGetDto>> DeleteBonus_CardAsync(Bonus_cardPutDto dto)
        {
            var data = await GetBonus_CardByIdAsync(dto.Barkod);
            if (data != null)
            {
        ...
        public async Task<Bonus_Card_ReportGetDto> DeleteBonus_Card_ReportAsync(int Barkod, int id)
        {
            var data = await GetBonus_Card_ReportAsync(Barkod);
            if (data != null)
            {
                foreach (var item in data.Bonus_Card_Reports)
                {
                    if(item.Bonus_CardId==id)
                    {
                        data.Description = "IsDelete";
                    }
                }
            ...
            var response = await GetBonus_Card_ReportAsync(Barkod);
            return response; ;
        }
